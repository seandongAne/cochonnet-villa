...
> **2026-08-04 盘面可读性改造（本文其余章节保留为历史记录，以下为现行参数）：**
> 为解决"设置了旋转但正常游玩距离肉眼读不出"的反馈，吸积盘按电影化参考重构：运动可读性改由盘面结构承担（见下），共享载波周期在试验 4/6/10 秒后按人工验收**保留原 10/15/25 秒**（2:3:5 差速比不变，`OBSERVATORY_BLACK_HOLE_FLOW_PERIODS` 单点导出，音频桥自动跟随）；新增统一呈现尺度 `OBSERVATORY_BLACK_HOLE_PRESENTATION_SCALE = 1.35`（程序化 mesh `scale`、Kerr `massWorldScale`、Schwarzschild `opticalScale` 三路同步，42 m 锚点与走动视差不变）；Kerr/Schwarzschild 发光盘外径从 5.6/7.6 扩展为 **10.5 M**（atlas 已烘焙至 ~11.7 M，无需重新生成），盘面成为透镜场的主体而非乳白天空扭曲环中的小核；盘面着色加入**土星式静态环带**（方位角无关，逐半径均值不随时间变化，保住"不整圈脉冲"契约）与**随本地轨道速率平流的高频剪切条纹**（零均值载波），径向发光衰减指数 1.72→1.15 让外圈丝带保持金褐色发光，强蓝移内缘新增白蓝热色阶。`npm test` 271/271、`npm run build`、High/Kerr 路径 `loft-center` 机位 t0/t2/t4 浏览器序列（白热点 下左→下右→上方，单帧间隔即可读出旋转）与零控制台错误已复验。
