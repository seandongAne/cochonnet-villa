...
| `observatory-events.js` | Node-pure rare-celestial-events director: 3%/s frame-rate-independent hazard while stargazing, 13-event pool (流星雨/彗星/星云增强/黑洞凌日/超新星/火流星/卫星列车/行星合/极光/星座连线/月亮过境/千新星/UFO) with per-event zh journal lines, envelopes, cooldown, availability gating, release-fade cancellation, injectable RNG |
...
**Rare celestial events (稀有天象):** while genuinely stargazing (dark loft, stars ≥ 0.5 revealed, sky renderable), `observatory-events.js` rolls a frame-rate-independent 3%/s hazard (`1-(1-p)^dt`) to start one of **13** events (uniform selection, no rarity weighting): 流星雨 / 彗星经过 / 星云增强 / 黑洞凌日 / 超新星爆发 / 火流星 / 卫星列车 / 行星合 / 极光 / 星座连线 / 月亮过境 / 千新星涟漪 / 不明飞行物 — each a ~18–55 s smootherstep envelope followed by a 40 s cooldown. The director is stepped only inside the runtime's -1 frame loop; 11 events render via `observatory-sky-events.js` inside the sky group, the surge multiplies Portal emission, and the transit drives the **same** damped lens target as hidden-F (inheriting the full fail-soft ladder + audio) without touching React R/F state. The moon transit honestly washes out stars (hero ×0.65, Gaia faint ×0.2 at full moonlight). Ineligibility (relight, leaving L3, availability loss) cancels via a 0.45 s release that never resurrects. A small HUD caption (`.villa-map-rare-event`) names the active event. **Gotcha:** every sky-event billboard material must stay `DoubleSide` — the anchored tangent frames face outward, so FrontSide culls them for the viewer at the shell's centre.

**天象图鉴 (event journal):** every started event is recorded (count + first-seen date) via `observatory-event-journal.js` into versioned localStorage. A physical KayKit book on an L3 west-wall shelf (`m3-journal-shelf`/`m3-journal-book` placements) carries an E interaction (`observatory-event-journal` action type in world.js) that opens `ObservatoryEventJournal.jsx` — a modal catalogue where unseen events stay as ？？？ silhouettes; it follows the Q panel's pointer-lock release/suspend discipline.
