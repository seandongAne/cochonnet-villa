...
| `architecture-placements.js` | GLB **entrance** accent placement data (Phase 4) — door-arch, topiaries, porch railings, planters; same stamped shape, same `createFurniturePiece`. Door-arch is `solid:false` so the doorway stays walkable |
...
- **Shell polish** (`assets.js`) — a new `addBeveledBox` helper (mirrors `addBox`, uses `RoundedBoxGeometry` from `three/addons`, radius ≈0.04–0.08 clamped, falls back to a plain box for sub-~0.012 slivers) rounds the chunky shell boxes (perimeter/wing walls, lintel, upper walls, roofs, chimney, porch deck/steps, planters, 4 corner posts) — thin glass/mullions stay flat. Refined `createMaterials` (warmer trims + new keys `fascia`/`doorWood`/`stoneBase`/`baseboard`) and added finish detail: an **ajar door slab pair** in the gap (`villa-door-left/right`, swung open so the x∈[-5,5] path stays clear), glass-pane frames, roof **fascia/eave** boards, interior **baseboards**. **`assets.js` stays node-pure** — no `TextureLoader`/`document` (the node suite imports it). Wall centres/extents are UNCHANGED (colliders depend on them); bevels/detail are purely visual.
- **Entrance accents** (`architecture-placements.js` + `public/models/architecture/`) — 9 CC0 GLB pieces at the villa front: a Furniture-kit `doorArch` (non-solid → doorway passable) + 2 `pottedPlant` topiaries flanking the door, and City-Suburban `railing` ×4 (porch edges, solid) + `planter` ×2 (solid). Same stamped shape + `createFurniturePiece`, so shadows + colliders apply uniformly. Placed clear of the Phase-3 courtyard props and the x∈[-5,5] entry path.
