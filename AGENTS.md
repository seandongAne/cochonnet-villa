...
| `mushroom-interior.js` | Procedural three-storey round-tower interior plus player-scale fairy-light/bunting canopies; node-pure like `assets.js` |
...
| `furniture-placements.js` | GLB furniture placement data, per room (Kenney villa + KayKit mushroom tower). Stamps each record at load with derived `footprint` (world m), `floor`, `solid`, `noShadow` (+ a `FURNITURE_FOOTPRINTS` native-size map) that the shadow + collider layers read |
...
- **KayKit mushroom refurnish** (`public/models/mushroom-furniture/` + `furniture-placements.js`) — the complete 53-model CC0 KayKit Furniture Bits free pack is converted to self-contained GLB. The architecture is now a 2× pocket, while the 94-piece hearth/den/loft composition (33/31/30) retains its curated 1.6× X/Z anchor spread inside it, creating a 20% wall/circulation buffer without shrinking the metre-authored models: layered rugs, pantry/dressers, close seating, books, wall shelves, photos, lamps and cushions. KayKit uses base scale 1 (the Kenney villa stays at 2.2). Only substantial grounded pieces are solid; elevated clutter stays collider-free. Three low fairy-light/bunting strands per floor visually lower the ceiling and two warm point-light pools per floor replace the old flat central wash.
