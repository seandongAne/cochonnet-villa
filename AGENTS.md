...
- **Enterable mushroom house** (`world.js` + `mushroom-interior.js` + `furniture-placements.js`) — the door interaction carries an `action: { label, teleport }`; pressing **E** (handled in `controls.js` → `PlayerControls` reads the nearest hotspot) teleports into a three-storey round-tower interior **buried at y≈-80 under the house** (invisible from outside; XZ overlaps the courtyard, so every interior zone/collider is Y-banded). Mechanics: generic `floorZones` (XZ rect + Y activation band + eyeY) checked in `getMovementProfile` after stairs; stair zones gained optional `minY`/`maxY`. Two 2.4 m-wide flights (A east L1→L2, B west L2→L3, both ascending northward) with shortened side-rail colliders (flight ends stay enterable — the player radius is added to every collider, a narrower flight seals itself shut), under-stair blocks and rim guards, all Y-banded. Furniture floors 2/3/4 map to the interior levels in `furniture-colliders.js`; a porky naps on L1. The exterior `mushroom-house` collider is Y-scoped to `[0,30]`.
- **KayKit mushroom refurnish** (`public/models/mushroom-furniture/` + `furniture-placements.js`) — the complete 53-model CC0 KayKit Furniture Bits free pack is converted to self-contained GLB. The tower is tightened from the former 4× showroom to a still-magical 1.6× pocket, with 94 pieces across hearth/den/loft (33/31/30): layered rugs, pantry/dressers, close seating, books, wall shelves, photos, lamps and cushions. KayKit is metre-authored and uses base scale 1 (the Kenney villa stays at 2.2). Only substantial grounded pieces are solid; elevated clutter stays collider-free. Three low fairy-light/bunting strands per floor visually lower the ceiling and two warm point-light pools per floor replace the old flat central wash.
